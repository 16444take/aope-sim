...
# Settings for RL 
import torch
from ppo.cdic import PPO
import ppo.config_cdic as ppo_config
...
if __name__ == '__main__':
    ...
    file_names = [self_name, 'ppo\\cdic.py','ppo\\config_cdic.py']
    ...
    for learner in range(len(agt_names)):
        ...
        rl = PPO(state_dim = obs_dims[learner] + act_dim[learner] + 1, 
                     ...
                     value_state_dim = 177 + n_ship + n_type + 3 + 4, 
                     config = rl_config)
    ...
    for i in range(n_episode+1):            
        ...
        for k in range(len(agt_names)):
            log_step_str = str(i)+','+str(ret_ave)+','+str(ret_std)+ ','+str(epl_ave)+ ','+str(epl_std)\
                            ...
                            +','+str(rl_agents[k].clip_frac)+','+str(rl_agents[k].approx_kl)+','+str(rl_agents[k].ret_actual)+','+str(rl_agents[k].v_estim)\
                            +','+str(rl_agents[k].expl_var)
