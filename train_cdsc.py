...
# Settings for RL 
import torch
from ppo.cdsc import PPOActor
from ppo.cdsc import PPOCritic
import ppo.config_cdsc as ppo_config
...
if __name__ == '__main__':
    ...
    file_names = [self_name, 'ppo\\cdsc.py','ppo\\config_cdsc.py']
    ...
    for learner in range(len(agt_names)):
        ...
        if learner==0: 
            log_header = 'step,return,ret_std,ep_len,ep_std,loss_pi,advantage,loss_s,clip_frac,approx_kl,loss_vf,ret_actual,v_estim,expl_var'
            for i in range(rl_config.n_rollouts): log_header += ',ret_rol'+str(i+1)
        else: log_header = 'step,return,ret_std,ep_len,ep_std,loss_pi,advantage,loss_s,clip_frac,approx_kl'
        print(log_header, file=log_result[learner])

        # Set RL agent actor
        rl = PPOActor(state_dim = obs_dims[learner] + act_dim[learner] + 1, 
                      action_dim = act_dim[learner], 
                      config = rl_config)            
        rl.save(model_fname[learner]+'0.pt') # Save initial model
        rl_agents.append(rl)                                    
                    
    # Set centralized critic
    critic_name = os.path.join(save_dir, 'model_critic_rs'+str(n_seed)+'_ts'+str(torch_seed)+'_ep')
    rlCritic = PPOCritic(value_state_dim = 177 + n_ship + n_type + 3 + 4, config = rl_config)
    rlCritic.save(critic_name+'0.pt') # Save initial model
    ...
    for i in range(n_episode+1):     
        ...
        for q in range(n_repeat_ep):
            ...
            for k in range(len(agt_names)):
                for j in range(len(train_envs)):
                    temp_buffer, cum_rwd, ep_len = ray.get(train_envs[j].get_history.remote(k))
            ...
            for j in range(len(train_envs)):
                temp_buffer = ray.get(train_envs[j].get_critic_history.remote())
                rlCritic.buffer.state_values += temp_buffer.state_values
                rlCritic.buffer.vf_states += temp_buffer.vf_states
                rlCritic.buffer.returns += temp_buffer.returns
        ...
        # Reset critic to each environment
        [ray.get(train_envs[j].reset_rl_critic.remote(rlCritic)) for j in range(rl_agents[0].n_para)]

        for k in range(len(agt_names)):
            if k == 0:
                log_step_str = str(i)+','+str(ret_ave)+','+str(ret_std)+ ','+str(epl_ave)+ ','+str(epl_std)\
                            +','+str(rl_agents[k].loss_pi)+','+str(rl_agents[k].advantage)+','+str(rl_agents[k].loss_s)\
                            +','+str(rl_agents[k].clip_frac)+','+str(rl_agents[k].approx_kl)\
                            +','+str(rlCritic.loss_vf)+','+str(rlCritic.ret_actual)+','+str(rlCritic.v_estim)+','+str(rlCritic.expl_var)
            else:
                log_step_str = str(i)+','+str(ret_ave)+','+str(ret_std)+ ','+str(epl_ave)+ ','+str(epl_std)\
                            +','+str(rl_agents[k].loss_pi)+','+str(rl_agents[k].advantage)+','+str(rl_agents[k].loss_s)\
                            +','+str(rl_agents[k].clip_frac)+','+str(rl_agents[k].approx_kl)
