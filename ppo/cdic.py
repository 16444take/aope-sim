...
import numpy as np
import statistics
from . import config_cdic as config
import sys
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, value_state_dim, hid_unit, use_gpu=True):
...
class PPO:
    def __init__(self, state_dim, action_dim, value_state_dim, config:config.Config):
    ...
    def select_action(self, state, vf_state, frame, action_mask, b_stochastic=True):
        with torch.no_grad():
    ...
    def calc_return(self):
        if self.b_ave_trs:
            ...
            self.buffer.rewards = [x*100/n_rewards for x in l_rewards]
    ...
    def sample(self, np_indices):
        ...
        return old_states, old_vf_states, old_actions, old_action_mask, old_logprobs, old_state_values, returns, advantages
    ...
    def update(self, agt_name):
        ...
        for i in range(self.epochs):
            ...
            for start in range(0, len_rollouts, self.batch_size):
                ...
                old_states, old_vf_states, old_actions, old_action_mask, old_logprobs, old_state_values, returns, advantages = self.sample(minibatch_indices)                        
